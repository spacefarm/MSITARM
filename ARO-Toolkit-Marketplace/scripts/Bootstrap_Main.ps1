...
try
{
	...
	$aroBootstrapStatus = Get-AutomationVariable -Name 'Internal_BootStrapRunbookCompleted'
	
	Write-Output "Checking if the BootStrap runbook completed one run and need to be deleted"
	...
	# Bootstrap execution is completed as part of deployment. Delete the Bootstrap runbook now.
	
	if ($aroBootstrapStatus -eq "true")
	{
		...
		Write-Output "Removing Internal_BootStrapRunbookCompleted Variable..."
        
        Remove-AzureRmAutomationVariable -Name 'Internal_BootStrapRunbookCompleted' -ResourceGroupName $aroResourceGroupName -automationAccountName $automationAccountName -ErrorAction SilentlyContinue
    ...
    try
    {
        ...
        $runbookNameforStopVM = "AutoStop_VM_Child"
        $webhookNameforStopVM = "AutoStop_VM_ChildWebhook"
    ...
    try
    {
        ...
        {
            Write-Output "Executing Step-3 : Create schedule for AutoStop_CreateAlert_Parent runbook ..."    
    ...
    # Removed step 4 as the schedules for scheduledSnooze will be created as part of deployment.
    # It will be passed to the deployment as input parameters.
    ...
    # Removed step 5 for marketplace deployment as the autoupdate feature has been removed
    ...
    try
    {
        ...
        {
            Write-Output "Executing Step-6 : Create schedule for SequencedStartStop_Parent runbook ..."
