               ...
               try {
                ...
                foreach($disk in $using:Disks.values) {
                    ...
                    if($thisExists -eq $null) {
                       ...
                       $DiskExists =   get-disk  | ? {$_.Number -notin $UsedDiskArray.DiskNumber} | ? {$($_.Size/1GB) -eq $($disk.Disksize) -and ($_.FriendlyName -eq 'Microsoft Virtual Disk' -or $_.FriendlyName -eq 'Msft Virtual Disk')}  | Sort-Object DiskNumber | select -First 1
